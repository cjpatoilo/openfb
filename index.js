let openFB = (() => {
  ...
  let logoutURL = 'https://www.facebook.com/logout.php'
  ...
  let oauthRedirectURL = baseURL + '/oauthcallback.html'

  // Default Cordova OAuth redirect URL. Can be overriden in init()
  var cordovaOAuthRedirectURL =
  ...
  function init (params) {
    ...
    cordovaOAuthRedirectURL =
      params.cordovaOAuthRedirectURL || cordovaOAuthRedirectURL
  ...
  function login (callback, options) {
    ...
    var startTime
    var scope = ''
    var redirectURL = runningInCordova
      ? cordovaOAuthRedirectURL
      : oauthRedirectURL
    ...
    function loginWindowExitHandler () {
      ...
      if (loginCallback && !loginProcessed)
  ...
  function oauthCallback (url) {
    ...
    if (url.indexOf('access_token=') > 0) {
      ...
      if (loginCallback)
        loginCallback({
          status: 'connected',
          authResponse: { accessToken: options['access_token'] },
        })
    } else if (url.indexOf('error=') > 0) {
      queryString = url.substring(url.indexOf('?') + 1, url.indexOf('#'))
      options = parseQueryString(queryString)
      if (loginCallback)
        loginCallback({ status: 'not_authorized', error: options.error })
    } else {
  ...
  function logout (callback) {
    ...
    if (token) {
      logoutWindow = window.open(
        logoutURL + '?access_token=' + token + '&next=' + logoutRedirectURL,
        '_blank',
        'location=no,clearcache=yes',
      )
  ...
  function api (options) {
    ...
    url =
      'https://graph.facebook.com' + options.path + '?' + toQueryString(params)
  ...
  function toQueryString (options) {
    ...
    for (var i in options) {
      if (options.hasOwnProperty(i))
        parts.push(encodeURIComponent(i) + '=' + encodeURIComponent(options[i]))
    }
